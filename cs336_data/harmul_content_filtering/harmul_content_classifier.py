...
class HarmfulContentClassifier:
    def __init__(self):
        ...
        self._toxic_classifier = fasttext.load_model(TOXIC_CLASSIFIER_MODEL_PATH.resolve()._str)
        self.logger.info(f"Toxic classifier model loaded from {TOXIC_CLASSIFIER_MODEL_PATH.resolve()}")

        self._nsfw_classifier = fasttext.load_model(NSFW_CLASSIFIER_MODEL_PATH.resolve()._str)
    ...
    def classify_toxic(self, text: str) -> Tuple[str, float]:
        ...
        labels, confidence_arr = self._toxic_classifier.predict(text)
        label = labels[0]
    ...
    def classify_nsfw(self, text: str) -> Tuple[str, float]:
        ...
        labels, confidence_arr = self._nsfw_classifier.predict(text)
        label = labels[0]
        confidence = confidence_arr[0]
